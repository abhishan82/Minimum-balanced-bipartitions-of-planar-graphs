...
\label{prop:warmup}
\lean{MinBal.prop_4_1}
% NOTE (2026-07-21): Lean's sep_tri_bipartition states the weaker +2 form
% with no biconnectivity requirement on either part, vs. the +1 form with
% G[V_1] biconnected stated here. Flagged for the author, not silently
% changed -- may be an intentional weakening that still feeds the final
% +2 bound of Theorem 1.1. See session log 2026-07-21.
...
\section{Sink Navigation (Section 5)}

\begin{definition}[Sink Data]
\label{def:sinkdata}
\lean{MinBal.SinkData}
\uses{prop:warmup}
The data extracted from the directed std-tree decomposition once no balanced separating triangle exists: the sink component $S$ (itself a plane triangulation with no separating triangle of its own), a separating triangle of $G$ witnessing $S$, the sink's degree in the directed std-tree, and the interval navigation data used in the degree-1 case (Proposition~\ref{prop:onedegsink}).
\end{definition}

\begin{proposition}[Sink Navigation]
\label{prop:sinknav}
\lean{MinBal.sink_navigation}
\uses{def:sinkdata, lem:4con, lem:3con1}
\notready
If $G$ is a plane triangulation whose sink $S$ has directed-std-tree degree at least 2 and $|V(S)| \geq 6$, then there exists a balanced bipartition $(V_1, V_2)$ of $V(G)$ satisfying the conclusion of Theorem~\ref{thm:main}. This is the technical core of the paper (Section 5), using the navigation Lemmas~\ref{lem:4con} and~\ref{lem:3con1} to build the bipartition incrementally. \textbf{Not yet formalized} -- the main outstanding mathematical content of this repository.
\end{proposition}
...
\begin{corollary}[Folklore Conjecture]
\label{cor:folklore}
\lean{MinBal.folklore_conjecture}
\uses{thm:main}
